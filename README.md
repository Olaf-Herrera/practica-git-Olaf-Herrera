...
* Desde la página de GitHub, edite directamente el archivo:
    * datos.txt
* Agrege una nueva línea escribí **"Este archivo fue modificado desde GitHub".**
...
* Modifique nuevamente el archivo **datos.txt** desde la computadora Agregando una nueva línea donde escribí **"Este archivo fue modificado desde el repositorio local".**
...
Se crea una carpeta con el nombre **practica-git-nombre-apellido** para luego entrar e inicir el repocitorio con el comando `git init`.

---

# Explicación de cómo se vinculó el repositorio local con GitHub
Se crea un repositorio en GitHub con el mismo nombre del repositorio local para luego copiar el URL, entrar a la terminal y poner desde el repositorio local `git remote add origin URL_DEL_REPOSITORIO` y con `git remote -v` para Verificar que el repositorio remoto se haya agregado correctamente.

---

# Explicación de la sincronización Local → GitHub
Con el Comando `git push -u origin main` por primera vez el repositorio local se enviara a GitHub. Para subir cambios se requiere preparar los cambios con `git add .` o `indicando la ruta del archivo específico` se agragan a la zona Staging Area, se guardar el Commit con `git commit -m "Mensaje descriptivo"`, despues utiliza el comado `git push` para subirlo.

---

# Explicación de la sincronización GitHub → Local
Para descargar los cambios que se hicieron directamente en GitHub hacia tu computadora se utiliza el Comando `git pull origin main`.
...
No es tan complicado crear un repositorio y vincularlo, yo no tuve problemas en ningun paso y la practica se me hiso intererante, con esto creo que ya me quedo claro como hacer repositorios, vincularlos y realizar basicamente un formato .md.
